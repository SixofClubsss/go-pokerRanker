...
func compareAll() { /// Main compare to determine winner
	...
	if totalHands >= 2 { /// Print results
		fmt.Println("Player 1 has:", cardEquiv(p1HandRaw[0]), cardEquiv(p1HandRaw[1]), cardEquiv(p1HandRaw[2]), cardEquiv(p1HandRaw[3]), cardEquiv(p1HandRaw[4]), p1HandRaw, handToText(p1Rank))
		fmt.Println("Player 2 has:", cardEquiv(p2HandRaw[0]), cardEquiv(p2HandRaw[1]), cardEquiv(p2HandRaw[2]), cardEquiv(p2HandRaw[3]), cardEquiv(p2HandRaw[4]), p2HandRaw, handToText(p2Rank))
	}
	if totalHands >= 3 {
		fmt.Println("Player 3 has:", cardEquiv(p3HandRaw[0]), cardEquiv(p3HandRaw[1]), cardEquiv(p3HandRaw[2]), cardEquiv(p3HandRaw[3]), cardEquiv(p3HandRaw[4]), p3HandRaw, handToText(p3Rank))
	}
	if totalHands >= 4 {
		fmt.Println("Player 4 has:", cardEquiv(p4HandRaw[0]), cardEquiv(p4HandRaw[1]), cardEquiv(p4HandRaw[2]), cardEquiv(p4HandRaw[3]), cardEquiv(p4HandRaw[4]), p4HandRaw, handToText(p4Rank))
	}
	if totalHands >= 5 {
		fmt.Println("Player 5 has:", cardEquiv(p5HandRaw[0]), cardEquiv(p5HandRaw[1]), cardEquiv(p5HandRaw[2]), cardEquiv(p5HandRaw[3]), cardEquiv(p5HandRaw[4]), p5HandRaw, handToText(p5Rank))
	}
	if totalHands >= 6 {
		fmt.Println("Player 6 has:", cardEquiv(p6HandRaw[0]), cardEquiv(p6HandRaw[1]), cardEquiv(p6HandRaw[2]), cardEquiv(p6HandRaw[3]), cardEquiv(p6HandRaw[4]), p6HandRaw, handToText(p6Rank))
	}
	fmt.Println()
	...
	if p1Rank < p2Rank && p1Rank < p3Rank && p1Rank < p4Rank && p1Rank < p5Rank && p1Rank < p6Rank { /// Outright win, player has highest rank
		...
		if p1HighPair > p2HighPair && p1HighPair > p3HighPair && p1HighPair > p4HighPair && p1HighPair > p5HighPair && p1HighPair > p6HighPair { /// No outright win, highest pairing first used to compare two hands of same rank
